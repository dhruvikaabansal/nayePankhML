...
<style>
  ...
  *{box-sizing:border-box}
  ...
  @media(max-width:860px){
    ...
    .steps,.grid3{grid-template-columns:1fr}.hero h1{font-size:27px}.twocol{columns:1}
  }
</style>
</head>
<body>
...
<div class="wrap">
  ...
  <section>
    ...
    <h2>Re-weight the Need Index for your campaign</h2>
    <p class="lead">Running a menstrual-hygiene drive? Slide that weight up and the ranking re-sorts live.
       This lets NayePankh target the <i>same</i> data differently for each programme.</p>
    ...
    <div class="tablewrap">
      <table id="tbl">
        <thead><tr>
          ...
          <th data-k="residual" class="num">Access gap</th>
        </tr></thead>
        <tbody></tbody>
      </table>
    </div>
    <p class="note" style="margin-top:8px">“Access gap” = actual menstrual-hygiene use minus what the model predicts
  ...
  <section>
    ...
    <p class="lead">Each model predicts a district's menstrual-hygiene use from its other indicators, scored with
       leave-one-out cross-validation (trained on 49 districts, tested on the 1 held out, repeated 50×). Higher
       R² and lower error = better.</p>
    <div class="grid2" style="margin-top:16px">
...
function renderSliders(){
  ...
  document.querySelectorAll('#sliders input').forEach(inp=>{
    inp.addEventListener('input',e=>{
      ...
      document.getElementById("w_"+k).textContent=e.target.value; recompute();
    });
  });
}
